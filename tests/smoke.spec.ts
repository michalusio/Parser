...
import { json_sample1k } from './smoke-data';
...
describe('JSON', function() {
    it(`should be parsed`, () => {
        ...
        const value: Parser<ValueType> = any(string, numberParser, boolP, objectParser, array, nullParser);

        function element(): Parser<ValueType> {
            return (ctx: Context): Result<ValueType> => between(wspaces, value, wspaces)(ctx);
        }

        // Act
        const result = element()(ctx);
